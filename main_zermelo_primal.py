...
"""
from Zermelo.ZermeloPrimalOCP import ZermeloPrimalOCP
import numpy as np
...
def main_primal():
    ...
    alpha = .9
    tol = 2e-5
    ocp = ZermeloPrimalOCP()
    ocp.set_eps(eps)
    options = dict(control_odes_error=True, display=1, res_tol=1e-6, abs_tol=1e-10, newton_tol=1e-12)
    ...
    while not convergence:
        ...
        if not infos.success:
            raise Exception("Primal Zermelo problem failed before convergence")
    ...
    with open("Zermelo/results_zermelo_primal.pickle", "wb") as fh:
